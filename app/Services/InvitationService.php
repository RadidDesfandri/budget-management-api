...
class InvitationService
{
    ...
    {
        if (
            $this->organizationUserRepo->isMember(
                $data["email"],
                $organizationId,
            )
        ) {
        ...
        if (
            $this->invitationRepo->hasActiveInvitation(
                $data["email"],
                $organizationId,
            )
        ) {
    ...
    public function acceptInvitation($invitation, $user)
    {
        return DB::transaction(function () use ($invitation, $user) {
            ...
            $isMember = $this->organizationUserRepo->isMemberByUserId(
                $user->id,
    ...
    {
        return $this->invitationRepo->getPaginatedInvitations(
            $currentUser,
            $filters,
        );
    }
